...
function depends_gzdoom() {
    ...
    cd zmusic_build && git clone https://github.com/coelckers/ZMusic.git zmusic && mkdir -pv zmusic/build
    cd zmusic_build/zmusic/build && cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr && make
    cd zmusic_build/zmusic/build && sudo make install
