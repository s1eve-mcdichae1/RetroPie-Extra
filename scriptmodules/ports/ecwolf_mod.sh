...
function _add_games_ecwolf_mod(){
    local ecw_bin="$1"
    local ext path game
    ...
    for game in "${!games[@]}"; do
        ...
        addPort "$md_id" "ecwolf" "${games[$game]}" "pushd $romdir/ports/wolf3d; bash %ROM% --fullscreen --res %XRES% %YRES%; popd" "$romdir/ports/wolf3d/${games[$game]}.ecwolf"
        _add_ecwolf_files_ecwolf_mod "$romdir/ports/wolf3d/${games[$game]}.ecwolf" "$path" "$ext" "$ecw_bin"
    done
    popd
}

function _add_ecwolf_files_ecwolf_mod() {
cat >"$1" <<_EOF_
cd "$2"
"$4" --data $3
wait \$!
...
function add_games_ecwolf_mod() {
    _add_games_ecwolf_mod "$md_inst/ecwolf"
}

function configure_ecwolf_mod() {
    ...
    [[ "$md_mode" == "install" ]] && game_data_ecwolf_mod
    [[ "$md_mode" == "install" ]] && add_games_ecwolf_mod
